...
void BitmapFont::show(double x,double y,string text,short font_color,double opacity,double scale_x,double scale_y){
    //Temporary offsets.
    double X=x,Y=y;
    ...
    for(short show=0;text[show]!='\0';show++){
        ...
        if(text[show]!='\xA'){
            //Show the character.
            render_font((int)X,(int)Y,bitmap_font_texture,&chars[ascii],font_color,opacity,scale_x,scale_y);
